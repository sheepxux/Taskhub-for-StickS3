...
并在来源支持时显示 token 或回合信息。BtnB 可以打开任务来源 App，BtnA
...
| 语音输入 | Ready | 长按 BtnB 说话（普通话/英文）→ 本地 whisper.cpp 转写 → 文字填入选中任务对应 App（`POST /voice`） |
...
在 StickS3 上**长按 BtnB** 即可对正在用的 AI App 说话——普通话或英文，
本地转写后直接填进对话框。

1. 短按 BtnB 打开某任务的 App（比如把 Claude 切到前台）。
2. **长按 BtnB** 说话，松手发送。
3. 音频 POST 到 `POST /voice`，由常驻 whisper.cpp 服务转写
   （`large-v3-turbo-q5_0`，简体中文 + 英文），再粘进该任务的 App。
   `?task=<id>` 会**确定性定向**到那个 App，文字必进你打开的窗口，不串窗口。
...
./host/install_whisper_server.sh   # 常驻 whisper-server LaunchAgent（:8080）
```

需要给 Host 开 **辅助功能(Accessibility)** 权限（系统设置 → 隐私与安全性 →
辅助功能），它才能向其它 App 粘贴。可调：`TASK_HUB_WHISPER_MODEL`、
`TASK_HUB_WHISPER_LANGUAGE`（`auto`/`zh`/`en`）、`/voice` 加 `?enter=1` 自动发送。
...
| 操作 | 功能 |
| --- | --- |
| BtnB | 打开当前选中任务的来源 App |
| 长按 BtnB | 语音输入——按住说话，松手转写并填入任务对应 App |
| BtnA | 切换到下一个任务 |
| 长按 BtnA | 立即刷新 |
