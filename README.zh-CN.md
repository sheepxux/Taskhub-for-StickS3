...
| WAIT 提醒 | Ready | 任务首次需要输入时边沿触发：点亮屏幕 + 琥珀闪烁 + 蜂鸣（`ALERT_*` 可调） |
...
| `WAIT` | 黄色 | 等待用户输入或需要注意 | 永远显示、保持屏幕唤醒，并在进入时触发一次提醒（点亮 + 闪烁 + 蜂鸣） |
| `FAIL` | 红色 | 失败或需要处理 | 永远显示 |
| `DONE` | 绿色 | 已完成 | 默认 10 分钟后隐藏 |
...
设备端 WAIT 提醒可在 `firmware/task_monitor/secrets.h` 调整：

| 设置 | 默认值 | 作用 |
| --- | --- | --- |
| `ALERT_ON_WAIT` | `1` | WAIT 提醒总开关 |
| `ALERT_BEEP` | `1` | 进入时蜂鸣；设 `0` 则只点亮屏幕、不出声 |
| `ALERT_BEEP_HZ` / `ALERT_BEEP_VOLUME` | `2600` / `160` | 蜂鸣音调与音量 |
